...
const DARK: Theme =
    Theme { bg: Rgb::hex(0x0b0e14), ink: Rgb::hex(0xf0f6fc), stripe: Rgb::hex(0x3d4451), label: Rgb::hex(0x8a94a6) };
const LIGHT: Theme =
    Theme { bg: Rgb::hex(0xffffff), ink: Rgb::hex(0x0b0e14), stripe: Rgb::hex(0xb8c0cc), label: Rgb::hex(0x6b7380) };
...
fn draw(layers: &mut Layers, theme: Theme) {
    let dark = Paint::dithered(Rgb::hex(0x000000), 0.6);
    ...
    Bubble::speech("layers").fill(Rgb::hex(0x161b22)).border(1.0, theme.label).ink(theme.ink).draw(
        top,
        1.5 * PANEL_W,
        PANEL_H - 8.0,
    );
